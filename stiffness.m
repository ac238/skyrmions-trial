...
function stiff = stiffness(m)
    ...
    centered_m = m(2:N-1,2:N-1,mod(1:3,3)+1).*(m(1:N-2,2:N-1,mod(2:4,3)+1)+m(3:N,2:N-1,mod(2:4,3)+1)+m(2:N-1,1:N-2,mod(2:4,3)+1)+m(2:N-1,3:N,mod(2:4,3)+1)-4*m(2:N-1,2:N-1,mod(2:4,3)+1)) - m(2:N-1,2:N-1,mod(2:4,3)+1).*(m(1:N-2,2:N-1,mod(1:3,3)+1)+m(3:N,2:N-1,mod(1:3,3)+1)+m(2:N-1,1:N-2,mod(1:3,3)+1)+m(2:N-1,3:N,mod(1:3,3)+1)-4*m(2:N-1,2:N-1,mod(1:3,3)+1));
    stiff = zeros(N,N,3);
